...
/**
 * Publishes case.outbound events and waits for the broker to settle the message before
 ...
 * correlation attributes and a span link to the most recent prior opposite-direction
 * event of the same case (plan §5.4).
 */
@ApplicationScoped
public class CaseEventPublisher {

    @Inject
    @Channel("case-outbound-out")
    ...
    public void publishOutbound(MessageRecord record, TraceContextRef linkTarget) {
        ...
        try (Scope ignored = span.makeCurrent()) {
            TraceAttributes.annotate(span, record.caseId(), record.eventId(),
                    record.direction(), record.author(), CaseEvents.ADDRESS_OUTBOUND);
            ...
            throw new PublishFailedException("Publishing case.outbound event failed", e);
        } finally {
            span.end();
        }
    }
