...
- Repository state: 115c129 Promote Phase 2 to k3s: Postgres+Keycloak in chart, secrets out of git, requirements validation (branch phase2)
...
| REQ-F-006 | FAIL | before='GREETING=value-one' after='GREETING=value-one' |
...
| REQ-F-007 | PASS | case c7a21b52-525d-43ba-9617-58486a150129 readable after deleting every app pod (data on PVCs: case-poc-anonymous-case-poc-artemis-data=Bound case-poc-anonymous-case-poc-postgres-data=Bound ) |
...
| REQ-F-003 | FAIL | 0 of 0 probes were non-200 during the upgrade |
| REQ-O-004 | MEASURED | helm upgrade -> all pods Ready: 7s (TBM threshold 180s => within) |
| REQ-F-004 | FAIL | rollback completed but application answered 502 |
| REQ-O-005 | MEASURED | helm rollback -> all pods Ready: 7s (TBM threshold 120s => within) |
...
| REQ-G-002 | PASS | create+delete of audit-probe-1783408355 present in audit.log (5 entries, path /var/lib/rancher/k3s/server/logs/audit.log) |
...
| REQ-G-007 | PASS | deploy/ history in git (last: 115c129 Promote Phase 2 to k3s: Postgres+Keycloak in chart, secrets out of git, requirements validation); Argo CD selfHeal=true reverts out-of-band mutations; API changes in audit.log (see REQ-G-002) |
...
| REQ-O-001 | MEASURED | k3s RSS 1073MiB + containerd/shims 476MiB; kube-system pods 71Mi, argocd pods 157Mi; idle CPU 4.2% (VM 4 vCPU) — TBM baseline recorded; catalogue threshold (512MB/5%) to be refined against it |
...
| REQ-O-002 | MEASURED | k3s restart -> node Ready + coredns/traefik/metrics-server available: 10s; app workload re-ready: 11s (TBM threshold 300s => within; VM boot excluded — k3s is a systemd unit) |

Requirements without an automatable criterion in this run: none — all 26
catalogue entries above. FAIL count: 3.
