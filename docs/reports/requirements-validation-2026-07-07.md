...
- Cluster: k3s v1.36.2+k3s1 on case-poc (x86_64), node IP 172.23.79.216
- Repository state: c7a8b75 phase2 implementation (branch phase2)
...
| REQ-F-007 | PASS | case 195f2557-1835-4649-9399-7b215acc6304 readable after deleting every app pod (data on PVCs: case-poc-anonymous-case-poc-artemis-data=Bound case-poc-anonymous-case-poc-postgres-data=Bound ) |
| REQ-F-001 | PASS | helm install of the multi-service chart completed; all pods Ready (release req, ns req-helm) |
| REQ-O-003 | MEASURED | helm install -> all pods Ready: 16s (images pre-imported; TBM threshold 180s => within) |
...
| REQ-O-005 | MEASURED | helm rollback -> all pods Ready: 6s (TBM threshold 120s => within) |
...
| REQ-G-002 | PASS | create+delete of audit-probe-1783449095 present in audit.log (5 entries, path /var/lib/rancher/k3s/server/logs/audit.log) |
...
| REQ-G-004 | PASS | API server presents auto-provisioned TLS cert on 6443 (subject=O = k3s, CN = k3s notAfter=Jul  7 17:16:11 2027 GMT ); kubeconfig pins the cluster CA |
...
| REQ-O-001 | MEASURED | k3s RSS 1095MiB + containerd/shims 485MiB; kube-system pods 65Mi, argocd pods 163Mi; idle CPU 3.0% (VM 4 vCPU) — TBM baseline recorded; catalogue threshold (512MB/5%) to be refined against it |
...
| REQ-O-002 | MEASURED | k3s restart -> node Ready + coredns/traefik/metrics-server available: 8s; app workload re-ready: 9s (TBM threshold 300s => within; VM boot excluded — k3s is a systemd unit) |
